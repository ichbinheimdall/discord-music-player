...
if (!client.player.getQueue(message)) return message.channel.send(new MessageEmbed().setColor("#EB459E").setAuthor(message.author.tag, message.author.avatarURL({ dynamic: true })).setDescription("**<:red:909510496284196874> Şu anda şarkı çalmıyor!**").setTimestamp().setFooter(`${config.EmbedFooter}`));
