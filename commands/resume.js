...
if (!message.member.voice.channel) return message.channel.send( new MessageEmbed().setColor("#EB459E").setAuthor(message.author.tag, message.author.avatarURL({ dynamic: true })).setDescription("**<:red:909510496284196874> Lütfen bir sesli kanala girin!**").setTimestamp().setFooter(`${config.EmbedFooter}`));
...
message.channel.send( new MessageEmbed().setColor("#EB459E").setAuthor(message.author.tag, message.author.avatarURL({ dynamic: true })).setDescription(`<:onay:909504199992168468> ${client.player.getQueue(message).playing.title} isimli şarkı başarıyla oynatılmaya devam ediyor.`).setTimestamp().setFooter(`${config.EmbedFooter}`));
