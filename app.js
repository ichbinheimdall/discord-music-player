...
client.player.on("botDisconnect", (message) => message.channel.send(new MessageEmbed().setColor("RANDOM").setDescription("**<:red:909510496284196874>️ - Müzik bittiği için sesli kanaldan ayrıldım.**").setTimestamp().setFooter(`${BotConf.EmbedFooter}`)));
...
client.player.on('noResults', (message, query) => message.channel.send(new MessageEmbed().setColor("RANDOM").setDescription(`**<:red:909510496284196874>️ - ${query} isimli şarkı YouTube'da bulunamadı!**`).setTimestamp().setFooter(`${BotConf.EmbedFooter}`)));
...
client.player.on('queueEnd', (message) => message.channel.send(new MessageEmbed().setColor("RANDOM").setDescription(`**<:red:909510496284196874>️ - Oynatma listesinde şarkı olmadığı için müzik durduruldu.**`).setTimestamp().setFooter(`${BotConf.EmbedFooter}`)));

client.player.on('searchCancel', (message) => message.channel.send(new MessageEmbed().setColor("RANDOM").setDescription(`**<:red:909510496284196874>️ - Geçerli bir argüman girmediniz! Lütfen tekrar deneyin.**`).setTimestamp().setFooter(`${BotConf.EmbedFooter}`)));

client.player.on('searchInvalidResponse', (message, tracks) => message.channel.send(new MessageEmbed().setColor("RANDOM").setDescription(`**<:red:909510496284196874>️ - Lütfen **1** ve **${tracks.length}** arasında bir sayı giriniz!**`).setTimestamp().setFooter(`${BotConf.EmbedFooter}`)));
...
client.player.on('error', (message, error) => { switch (error) {
  case 'NotPlaying': message.channel.send(new MessageEmbed().setColor("RANDOM").setDescription("**<:red:909510496284196874> Bu sunucuda müzik çalamıyorum!**").setTimestamp().setFooter(`${BotConf.EmbedFooter}`)); break;
  case 'NotConnected': message.channel.send(new MessageEmbed().setColor("RANDOM").setDescription("**<:red:909510496284196874> Şu anda bir sesli kanalda bulunmamaktasınız!**").setTimestamp().setFooter(`${BotConf.EmbedFooter}`)); break;
  case 'UnableToJoin': message.channel.send(new MessageEmbed().setColor("RANDOM").setDescription("**<:red:909510496284196874>️ - Bulunduğunuz kanala girmem için yeterli iznim yok, gerekli izinleri verdikten sonra lütfen tekrar deneyin.**").setTimestamp().setFooter(`${BotConf.EmbedFooter}`)); break;
  default: message.channel.send(new MessageEmbed().setColor("RANDOM").setDescription(`**<:red:909510496284196874>️ - Bir şeyler ters gitti, lütfen botun sahibiyle iletişime geçin...**`).setTimestamp().setFooter(`${BotConf.EmbedFooter}`));
}; });
